...
def build_annotation(rtmpose_root, label_map):
    ...
    for label_key, samples in sample_by_class.items():
        random.shuffle(samples) # Shuffle samples within each class
        
        n_samples = len(samples)
        n_val = max(1, n_samples // 10)     # Ensure at least 1 sample for val
        n_test = max(1, n_samples // 10)    # Ensure at least 1 sample for test
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--json_root', default='data/json_rtmpose_all_view_2D', help='Root directory of JSON files (e.g., data/json)')
    # parser.add_argument('--json_root', default='data/json_rtmpose_front', help='Root directory of JSON files (e.g., data/json)')
    parser.add_argument('--json_root', default='../data/json_all_view_plus_idle', help='Root directory of JSON files (e.g., data/json)')
    # parser.add_argument('--out_pkl', default='data/pickle/rtmpose_2D_all.pkl', help='Output path for the .pkl file (e.g., data/custom_data.pkl)')
    # parser.add_argument('--out_pkl', default='data/pickle/rtmpose_2D_front.pkl', help='Output path for the .pkl file (e.g., data/custom_data.pkl)')
    parser.add_argument('--out_pkl', default='data/pickle/rtmpose_2D_all_plus_idle.pkl', help='Output path for the .pkl file (e.g., data/custom_data.pkl)')
